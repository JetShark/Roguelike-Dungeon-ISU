...
public class SpriteRetrival {
    ...
    private static BufferedImage enemyProjectileSprites;
    ...
    public static BufferedImage getSprite(int xGrid, int yGrid, int spriteNumber){
        ...
        if(spriteNumber == 9){
            return getEnemyProjectileSpriteSheet(xGrid, yGrid);
        }
        return null;
    }
    ...
    private static BufferedImage getEnemyProjectileSpriteSheet(int xGrid, int yGrid){
        if(enemyProjectileSprites == null){
            enemyProjectileSprites = loadSprite("Enemy-Projectiles-SpriteSheet");
        }

        int key=xGrid+yGrid*TILES_ACROSS+9*TILES_ACROSS*TILES_DOWN;
        BufferedImage subimage;
        if (cachedImage.containsKey(key)) {
            subimage = cachedImage.get(key);
        } else {
            subimage = enemyProjectileSprites.getSubimage(xGrid * TILE_SIZE, yGrid * TILE_SIZE, TILE_SIZE, TILE_SIZE); // returns the sub images based on the x,y values given.
            cachedImage.put(key, subimage);
        }
